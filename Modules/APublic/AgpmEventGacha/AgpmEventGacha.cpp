...
BOOL	AgpmEventGacha::OnAddModule()
{
	...
	if (!m_pcsApmEventManager || !m_pcsAgpmCharacter || !m_pcsAgpmFactor || !m_pcsApmObject )
		return FALSE;
	...
	case	OP_REQUEST	:	// 가챠 요청 Client->Server
		{
			...
			else
			{
				...
				*/
				...
				if( stGachaInfo.pcsEvent->m_pcsSource->m_eType == APBASE_TYPE_OBJECT )
				{
					ApdObject *pObject = m_pcsApmObject->GetObject(stGachaInfo.pcsEvent->m_pcsSource->m_lID);
					if(pObject)
						m_pcsAgpmCharacter->MoveCharacterFollow(pAgpdCharacter, pObject, AGPMEVENT_GACHA_MAX_USE_RANGE, FALSE, NULL, FALSE);
				}
			}
		}
		break;
