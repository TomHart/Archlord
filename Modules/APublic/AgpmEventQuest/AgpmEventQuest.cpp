...
BOOL AgpmEventQuest::OnAddModule()
{
	...
	if(!m_pcsAgpmCharacter || !m_pcsApmEventManager || !m_pcsApmObject)
		return FALSE;
...
BOOL AgpmEventQuest::OnOperationRequest(ApdEvent* pcsEvent, AgpdCharacter* pcsCharacter, INT32 lRange)
{
	...
	else
	{
		...
		{
			ApdObject *pObject = m_pcsApmObject->GetObject(pcsEvent->m_pcsSource->m_lID);
			if(pObject)
				m_pcsAgpmCharacter->MoveCharacterFollow(pcsCharacter, pObject, AGPMEVENT_QUEST_MAX_USE_RANGE, FALSE, NULL, FALSE);
		}
	}
