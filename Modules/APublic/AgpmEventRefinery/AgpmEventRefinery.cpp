...
BOOL AgpmEventRefinery::OnAddModule()
	...
	if (!m_pApmEventManager || !m_pAgpmCharacter || !m_pApmObject)
		return FALSE;
...
BOOL AgpmEventRefinery::OnOperationRequest(ApdEvent *pApdEvent, AgpdCharacter *pAgpdCharacter)
	...
	if (m_pApmEventManager->CheckValidRange(pApdEvent, &pAgpdCharacter->m_stPos, AGPMEVENTREFINERY_MAX_USE_RANGE, &stTargetPos))
		...
		if( pApdEvent->m_pcsSource->m_eType == APBASE_TYPE_OBJECT )
		{
			ApdObject *pObject = m_pApmObject->GetObject(pApdEvent->m_pcsSource->m_lID);
			if(pObject)
				m_pAgpmCharacter->MoveCharacterFollow(pAgpdCharacter, pObject, AGPMEVENTREFINERY_MAX_USE_RANGE, FALSE, NULL, FALSE);
		}
		}
