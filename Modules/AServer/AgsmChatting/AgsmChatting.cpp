...
BOOL AgsmChatting::ParseCommand(AgpdChatData * pstChatData)
{
	...
	if (pstChatData->szMessage[i] == '/')
	{
		...
		else if (strncmp(pstChatData->szMessage + i, ServerStr().GetStr(STI_CHAT_MOVE_SPEED_UP), j - i) == 0)
		{
		...
		else if (strncmp(pstChatData->szMessage + i, ServerStr().GetStr(STI_CHAT_MOVE_SPEED_DOWN), j - i) == 0)
		{
			if (pstChatData->pcsSenderBase &&
				pstChatData->pcsSenderBase->m_eType == APBASE_TYPE_CHARACTER &&
				((AgpdCharacter *) pstChatData->pcsSenderBase)->m_pcsCharacterTemplate)
		...
		else if (strncmp(pstChatData->szMessage + i, "/exptoggle", max(strlen("/exptoggle"), j - i)) == 0)
		{
			...
			pcsAgsdCharacter->m_bDisableExp = !pcsAgsdCharacter->m_bDisableExp;

			if(m_pagsmSystemMessage){
				m_pagsmSystemMessage->SendSystemGeneralString(
					pcsAgpdCharacter, 
					pcsAgsdCharacter->m_bDisableExp ? "Exp Disabled" : "Exp Enabled"
				);
			}

			return TRUE;
		}
		else if (strncmp(pstChatData->szMessage + i, "/rates", max(strlen("/rates"), j - i)) == 0)
		{
			AgpdCharacter* pcsAgpdCharacter = m_pagpmCharacter->GetCharacter(pstChatData->pcsSenderBase->m_lID);
