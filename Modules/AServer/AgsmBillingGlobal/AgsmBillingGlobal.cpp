...
BOOL AgsmBillingGlobal::ConnectShop()
{
	...
	eWZConnect stRetVal = m_pWZShop->Connect(pIPAddress, atoi(pPort));

	{
		CHAR strLog[256] = { 0, };
		sprintf_s(strLog, sizeof(strLog), "ConnectShop : pIPAddress = %s, pPort = %s, retVal = %d", pIPAddress, pPort, stRetVal);
		OnLog(strLog);
	}*/
...
BOOL AgsmBillingGlobal::ConnectBilling()
{
	...
	eWZConnect stRetVal = m_pWZBilling->Connect(pIPAddress, atoi(pPort));
	{
		CHAR strLog[256] = { 0, };
		sprintf_s(strLog, sizeof(strLog), "ConnectBilling : pIPAddress = %s, pPort = %s, retVal = %d", pIPAddress, pPort, stRetVal);
		OnLog(strLog);
	}*/
...
BOOL AgsmBillingGlobal::BuyProduct(CHAR* AccountID, INT32 ProductID, CHAR* ProductName, UINT64 ListSeq, INT32 Class, INT32 Level, CHAR* szCharName, INT32 ServerIndex, double Price, INT32 lType, DWORD AccessIP )
{
	return TRUE;
}
