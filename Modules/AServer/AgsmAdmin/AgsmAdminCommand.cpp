					...
					if(!strcmp(strParam, "event"))
					{
						...
						if(pLock.Result())
						{
							...
							if(m_pagsmSystemMessage){
								m_pagsmSystemMessage->SendSystemGeneralString(pcsAgpdCharacter, "Reloading EventConfig");
							}
						}
					}
			...
			if(lMoney > 0)
			{
				...
				else
				{
					...
					if(pcsTarget)
					{
						...
						char strCharBuff[256] = { 0, };
						sprintf_s(strCharBuff, sizeof(strCharBuff), "[MONEYTO] %s(%s) : %s %d\n",
							pcsAgpdCharacter->m_szID,
							pcsAgsdCharacter->m_szAccountID,
							szCharID,
							lMoney);
...
// 코드 아래쪽에 아크로드 결정전 명령어끼리 모여있는부분과 동일한 명령어이고 다른처리를 하고있어서 문제 수정 [ 2011-07-27 silvermoo ]
	...
	else if (strnicmp(pstChatData->szMessage + i, "/ExpRatio", k - i) == 0)
	{
		...
		if(m_pagsmSystemMessage){
			...
			sprintf( strSystemMessage, "Exp Ratio now at %f", m_pagpmConfig->GetExpAdjustmentRatio());
	...
	else if (strnicmp(pstChatData->szMessage + i, ServerStr().GetStr(STI_COMMAND_BANK), k - i) == 0)
	{
		...
		char strCharBuff[256] = { 0, };
		sprintf_s(strCharBuff, sizeof(strCharBuff),"[BankAnywhereOpen] %s(%s) : %s\n",
			bCheckAdmin ? pcsAgpdCharacter->m_szID : _T("From Console"),
			bCheckAdmin ? pcsAgsdCharacter->m_szAccountID : _T(""),
			szArg);
		AuLogFile_s(AGSMADMIN_COMMAND_LOG_FILE, strCharBuff);
