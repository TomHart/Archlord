...
BOOL AgsmConfig::LoadConfigLua()
{
	if(m_bReloadConfigLua)
	{
		...
		pluaPack.Open("INI\\Lua\\EventConfig.lua");
...
int AgsmConfig::SetDoubleEventState(lua_State* L)
{
	...
	pThis->m_pcsAgpmConfig->SetDrop2AdjustmentRatio((float)ItemDropRatio/100);
...
/*
	...
	for(AuXmlNode* pNode = pRoot->FirstChild("EventMessage"); pNode; pNode = pNode->NextSibling("EventMessage"))
	{
			...
			MessageNumber == atoi(pElemMessageNum->GetText()))
		{
			...
			const size_t arraySize = 50;  // Avoid magic numbers!
			char buffer[arraySize];
			sprintf(buffer, "ExpRatio  : %f",  pThis->m_pcsAgpmConfig->GetExpAdjustmentRatio());
			pThis->m_pagsmSystemMessage->SendSystemMessageAllUser(AGPMSYSTEMMESSAGE_CODE_GENERAL_STRING, -1, -1, buffer, NULL);

			memset(&(buffer[0]), 0, arraySize);
			sprintf(buffer, "DropRatio : %f",  pThis->m_pcsAgpmConfig->GetDropAdjustmentRatio());
			pThis->m_pagsmSystemMessage->SendSystemMessageAllUser(AGPMSYSTEMMESSAGE_CODE_GENERAL_STRING, -1, -1, buffer, NULL);

			memset(&(buffer[0]), 0, arraySize);
			sprintf(buffer, "Drop2Ratio: %f",  pThis->m_pcsAgpmConfig->GetDrop2AdjustmentRatio());
			pThis->m_pagsmSystemMessage->SendSystemMessageAllUser(AGPMSYSTEMMESSAGE_CODE_GENERAL_STRING, -1, -1, buffer, NULL);

			memset(&(buffer[0]), 0, arraySize);
			sprintf(buffer, "GoldRatio : %f",  pThis->m_pcsAgpmConfig->GetGheldDropAdjustmentRatio());
			pThis->m_pagsmSystemMessage->SendSystemMessageAllUser(AGPMSYSTEMMESSAGE_CODE_GENERAL_STRING, -1, -1, buffer, NULL);

			memset(&(buffer[0]), 0, arraySize);
			sprintf(buffer, "CharRatio : %f",  pThis->m_pcsAgpmConfig->GetCharismaDropAdjustmentRatio());
			pThis->m_pagsmSystemMessage->SendSystemMessageAllUser(AGPMSYSTEMMESSAGE_CODE_GENERAL_STRING, -1, -1, buffer, NULL);
		}
	}
